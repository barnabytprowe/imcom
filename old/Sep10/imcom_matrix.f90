...
SUBROUTINE imcom_get_Al(npoly, saveA)
...
call imcom_writefits('./fits/Alookup11.fits', n1psf, n2psf, Alookup(:, :, 1, 1))
call imcom_writefits('./fits/Alookup12.fits', n1psf, n2psf, Alookup(:, :, 1, 2))
call imcom_writefits('./fits/Alookup21.fits', n1psf, n2psf, Alookup(:, :, 2, 1))
call imcom_writefits('./fits/Alookup22.fits', n1psf, n2psf, Alookup(:, :, 2, 2))
!call imcom_build_Blookup
!call imcom_writefits('./fits/Blookup1.fits', n1psf, n2psf, Blookup(:, :, 1))
!call imcom_writefits('./fits/Blookup2.fits', n1psf, n2psf, Blookup(:, :, 2))

write(*, FMT='(A)') "IMCOM: Building A matrix"
allocate(Al_aij(n, n), STAT=alstat)
! Initialize
Al_aij = 0.d0
nperexp = n / nexp
iexp = 0
...
do i=1, n
  ...
  do j=i, n
    ...
    Delx = x_i(j) - x_i(i)
    Dely = y_i(j) - y_i(i)
    Al_aij(i, j) = imcom_interp_lookup(Alookup(:, :, iexp, jexp), Delx, Dely, npoly)
...
do i=2, n
  ...
  forall(j=1:i-1) Al_aij(i, j) = Al_aij(j, i)
...
write(*, FMT='(A)') "IMCOM: A matrix complete"
if (saveA.eq.1) then 
  write(*, FMT='(A)') "IMCOM: Saving A matrix to ./fits/Altest.fits"
  call imcom_writefits("./fits/Altest.fits", n, n, real(Al_aij, 8))
end if
END SUBROUTINE imcom_get_Al
...
real(KIND=8), intent(IN) :: xarg, yarg  ! x,y arg in physical units
...
iarg = icent + xarg / psfxscale
jarg = jcent + yarg / psfyscale
ia_floor = floor(iarg)
ja_floor = floor(jarg)
...
write(*, FMT='(A)') "IMCOM: Fourier transforming B vectors lookup table"
...
do iexp=1, nexp
  ...
  ufunc = imcom_upsilon2D(ux, uy, 1) * Gt_rot(:, :, iexp) * dconjg(Gammat(:, :))
...
do a=1, m
  ...
  integrand = real(imcom_upsilon(ux_m, uy_m, 1) * dconjg(Gterm - Gamterm) * (Gterm - Gamterm), 8)
