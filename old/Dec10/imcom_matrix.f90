...
integer :: iexp, jexp, i, j, alstat, nperexp, n1al, n2al
...
n2al = n2psf * npad
!$omp parallel
!$omp do schedule(dynamic, 32) private(Delx, Dely)
do i=1, n
  ...
  do j=i, n
    ...
    A_aij(i, j) = imcom_interp_lookup(n1al, n2al, Alookup(:, :, iexp, jexp), &
                                      Delx, Dely, npoly)
...
real(KIND=8) :: Delx, Dely
integer :: iexp, i, a, alstat, nperexp, n1bl, n2bl
...
do i=1, n
  ...
  do a=1, m
    ...
    B_ia(i, a) = imcom_interp_lookup(n1bl, n2bl, Blookup(:, :, iexp), Delx, &
                                     Dely, npoly)
...
!$omp parallel
!$omp do private(ufunc, A_tmp) collapse(2)
do iexp=1, nexp
...
write(*, FMT='(A)') "IMCOM: Writing projection matrix to "//trim(Pfile)
