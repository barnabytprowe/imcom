!    LAPD_DATA.F90 - Fortran 95 module that acts as a shared-data / COMMON 
!                    block for the prototype LAPD package
...
character(LEN=256) :: inPSfile, transferGfile

! output desired k file, image "H" and Gamma files
character(LEN=256) :: gamfile, Hfile
...
real(KIND=8), dimension(:), allocatable :: ell
real(KIND=8), dimension(:), allocatable :: C_ell
...
real(KIND=8), dimension(:), allocatable :: err_C_ell

! (Desired "PSF": usually the identity matrix so keep ID for now...)
real(KIND=8), dimension(:), allocatable :: Gamma_ab
