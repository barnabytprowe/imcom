!    LAPD_IO.F90 - Fortran 95 module that handles basic I/O for the
!                  prototype LAPD package
...
!
!    CONTAINS
!    Routines for LAPD that include FITSIO wrapper routines; usage error 
...
SUBROUTINE lapd_read_alloc_G
! Explore & read in the large file containing the transfer function 
implicit none
integer, dimension(:), allocatable :: nell, nk, bitpix
...
SUBROUTINE lapd_sizefits(filename, naxis1, naxis2, bitpix)
...
END SUBROUTINE lapd_sizefits

!---

SUBROUTINE lapd_readfits(filename, naxis1, naxis2, image)
...
if ((bitpix.ne.-64)) then
  write(*, '(A)') "LAPD WARNING: Input fits file not 64-bit floating point data... will probably end badly!"
end if
if (naxis1.ne.naxes(1)) then
  write(*, FMT='(A)') "LAPD ERROR: Specified NAXIS1 does not match up to image in "//trim(filename) 
  stop
endif
if (naxis2.ne.naxes(2)) then
  write(*, FMT='(A)') "LAPD ERROR: Specified NAXIS2 does not match up to image in "//trim(filename) 
  stop
endif
group = 1
firstpix = 1
...
call FTFIOU(unit, stat)       ! Free the I/O unit
call FTRPRT('STDERR', stat)   ! Error report
END SUBROUTINE lapd_readfits

!---

SUBROUTINE lapd_writefits(filename, naxis1, naxis2, image)
...
SUBROUTINE lapd_welcome
implicit none
! Welcome banner
write(*, FMT='(A)') "LAPD: Linear Algebra Power spectrum Deconvolution   Copyright B. Rowe 2013"
END SUBROUTINE lapd_welcome

!---

SUBROUTINE lapd_usage_message
implicit none
call lapd_welcome
write(*, '(A)') "LAPD: [usage]"
write(*, '(A)') " ./lapd <config_file> <U/S> <U/S_max> <U/S_tol> [...<forceT>]"
write(*, '(A)') " "
...
END SUBROUTINE lapd_get_cmdline
