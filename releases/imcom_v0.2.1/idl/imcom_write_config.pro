PRO imcom_write_config, CONFIGFILE=configfile, $
                        NEXP=nexp,             $
                        USERXY=userxy,         $
                        GIMFILE=gimfile,       $
                        GIMXFILE=gimxfile,     $
                        ...
                        NOUT=nout
;
;    IMCOM_WRITE_CONFIG.PRO - writes all config files for the
...
;
;    DEPENDENCIES - IMCOM_WRITE_INCONFIG.PRO
;                   IMCOM_WRITE_OUTCONFIG.PRO
;
;    DESCRIPTION
;    Writes all configuration info (supplied by keywords) to ASCII 
;    configuration files in the format in which IMCOM is guaranteed to
;    function correctly.  For a description of these keywords and the
;    values they should take, please refer to the supplied user manual
;    IMCOM_DOCUMENTATION.PDF
;
;
